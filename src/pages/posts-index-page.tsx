  ...
  return (
    <section className="mx-auto grid max-w-5xl gap-8 lg:grid-cols-[12rem_minmax(0,1fr)] lg:gap-12">
      <div className="space-y-3 lg:sticky lg:top-28 lg:self-start">
        <h1 className="type-page-title">Posts</h1>
        <p className="type-lede max-w-sm">Notes, recipes, and longer-form technical writing.</p>
      </div>
          ...
          <article key={post.slug} className="border-b p-5 last:border-b-0 sm:p-6">
            <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
              <div className="space-y-2">
                <a
                  className="type-ledger-title inline-flex min-h-11 items-center rounded-md hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-card"
                  href={`/posts/${post.slug}`}
                >
                  {post.title}
                </a>
                <p className="type-metadata">{formatDate(post.date)}</p>
              </div>
              <div className="flex flex-wrap gap-2">
                {post.tags.map((tag) => (
                  <a
                    key={tag}
                    className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-card"
